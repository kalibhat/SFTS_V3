...
#include "cl_types.h"
#include "cl_dprep_primecontroller.h"
#include "cl_dlsis_controller.h"
#include "cl_dprep_controller.h"
...
#include "cl_rinse_controller.h"
#include "cl_heatercontroller.h"
...
extern void DD_IIC_READ_PROP(Cl_Uint8Type iic_address, uint16_t* data);
extern Cl_Uint32Type Treatdata[ID_MAX_TREAT_PARAM];
extern void DD_IIC_SEND_PROP(Cl_Uint8Type iic_address, Cl_Uint8Type* data);
extern Sv_ReturnCodesType sv_nvmsetdata(Cl_Uint8Type dataItemId,Cl_Uint8Type* data,Cl_Uint8Type);
extern Sv_ReturnCodesType  sv_nvmgetdata(Cl_Uint8Type,Cl_Uint8Type*);
...
extern Cl_Uint8Type sv_cntrl_setpumpfreq(sv_pumptype sv_pump_id,Cl_Uint32Type freq);
extern Cl_Uint8Type sv_cntrl_activatevenousclamp(void);
extern Cl_Uint8Type sv_cntrl_deactivatevenousclamp(void);
...
extern Cl_AlarmActivateAlarms(Cl_NewAlarmIdType,Cl_BoolType);
...
extern Cl_Uint8Type sv_cs_readpotvalue(uint16_t* resistance);
extern Cl_ReturnCodeType 	Cl_Mac_GetSystem_Status_Query(void);
...
extern Cl_Uint8Type sv_cntrl_enable_loopback(void);
...
extern Cl_Uint8Type sys_sw_version;
...
extern Cl_AlarmStructType Cl_alarms_alarms[_ALARM_MAX_ID];
 ...
 Cl_BoolType current_sense = false;
 Cl_BoolType test_enabled = false;
 	
 Cl_Mac_StatesType Cl_Mac_StateDummy;
 ...
 Cl_Dlsis_States cl_dlsis_state_dummy;
  Cl_BoolType Loopbackstatus;
  Cl_BoolType Bypassstatus;

Cl_ReturnCodeType cl_testharnesscommandhandler(Cl_ConsoleMsgType*);
Cl_ReturnCodeType  cl_testgetsensordata(Cl_Uint8Type dataId, uint16_t* dataItem);
Cl_ReturnCodeType  cl_testgetversion( Cl_Uint8Type* );
Cl_Uint16Type lookup_condtable(Cl_Uint8Type cond_base,Cl_Uint16Type cond_mvolt);
Cl_BoolType TEST_STUB_ENABLEGATE = 1;
Cl_BoolType prev_gate_status =1;
int array_commands[100] = {};
Cl_NewAlarmIdType cl_testalarm_id ;
 Cl_BoolType heater_update_disble = false ;
 Cl_Sys_statusType  cl_sys_statbuffer_test;
volatile Cl_Uint8Type flag_hotcitric=0,flag_hotrinse=0,flag_na=0,flag_citric = 0;
extern Cl_Uint8Type hotrinse_flag;
void DD_IIC_SET_BLOODPUP(Cl_Uint8Type iic_address, Cl_Uint32Type data,Cl_Uint8Type length);
...
void testsectclock(void);
	static Cl_Uint16Type cond_lookup[20] =
	{
		780,//3
		1010,//4
		1210,//5
...
Cl_Uint16Type mix_count = 0;
Cl_BoolType BC_window = false;
Cl_BoolType start_pressurecapture = false;
Cl_BoolType pressuredataready = false;
Cl_BoolType g_testbcfreeze = false;
Cl_BoolType Current_sense_trigger = true;
Cl_BoolType dummy_currentsence = false;
Cl_BoolType dummy_levelswitchstatus = false;
Cl_BoolType gflow_en = false;
int16_t PS3_threshold = 0 ;
extern int16_t ps3_threshold ;
static int16_t ps1avg=0, ps2avg = 0,ps3avg = 0, aptavg=0,vptavg=0;
Cl_BoolType gtest_short_rinse = false ;
extern Cl_Uint32Type heater_duty ;
Cl_BoolType iic_nack = false;
...
Cl_ReturnCodeType cl_testharnesscommandhandler(Cl_ConsoleMsgType* pCl_ConsoleMsg)
{
	...
	Cl_Uint8Type dataarray[20];
	Cl_Uint8Type datatxarray[20];
...
//		sv_cntrl_setpumpspeed(UFPUMP,10000);
		...
		break;
		pdataarray = (Cl_Uint8Type *)"UFP_ON";
	//	cl_thretval = Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTTEXT,pdataarray,6);
	//	cl_thretval = Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTDATA,&TestMsg.data.byte[1],TestMsg.datasize-1);
		cl_wait(100);
		break;
	...
	//	Cl_Uint8Type bloodpumpspeed1[3];
	...
	//				cl_thretval = Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTTEXT,pdataarray,11);
		...
		//			cl_thretval = Cl_SendDatatoconsole(CON_TX_COMMAND_PRINTTEXT,pdataarray,11);
			...
			#if 0
			// gtest_short_rinse  = (Cl_BoolType) TestMsg.data.byte[1];
			{
							 Cl_Uint8Type  test_hstate;
							test_hstate = CL_HEATER_SUBSTATE_DISTRIBUTION;
							Cl_SendDatatoconsole(CON_TX_COMMAND_COMMAND_HEATER_STATE,(void*)&test_hstate,1);
							Cl_SendDatatoconsole(CON_TX_COMMAND_COMMAND_HEATER_STATE,(Cl_Uint8Type*)&test_hstate,1);
...
Cl_ReturnCodeType  cl_testgetversion( Cl_Uint8Type* version)
{
	Cl_ReturnCodeType cl_thretval = CL_ERROR;
...
void Cl_tg_prick_50ms(void)
{
	Cl_ReturnCodeType cl_thretval = CL_ERROR;
	int16_t Ps = 0;
	static Cl_Uint16Type pcnt =0;
	static Cl_Uint8Type cnt = 0;
	int16_t Pressuredatainmillibars =0,ps1Pressuredatainmillibars=0, sensordatamillivolts =0;
	static int16_t Pdata[100];
	static Cl_BoolType capture_started = false;
	static Cl_BoolType Pre_BC_window = false;
	static Cl_BoolType BC_FLAG = false;
    static Cl_BoolType start_flag = false;
...
{
	static Cl_Uint16Type pin_status = 1,uf_rotataion_marker = 1,prev_uf_rotataion_marker = 1;
	static Cl_Uint32Type fivemscounter=0,long_counter=0,uf_period = 0,uf_freq_in_min = 0, flow_count = 0, prev_flow_status=0;
	static Cl_BoolType count_state = false;
	static toggle = false;
	Cl_Uint8Type dataarray[3];
